            ...
            $().ready(function() {
                ...
                else
                {
                    var elf = $('#elfinder').elfinder({
                        url : 'php/connector.php?mode=' + mode,  // connector URL (REQUIRED)
                        getFileCallback : function(file) {
                            if($("#featured_image_patern", window.opener.document ).length>0){$("#featured_image_patern", window.opener.document ).val(file);}
                            if($("#featured_image_src_patern", window.opener.document ).length>0){$("#featured_image_src_patern", window.opener.document ).attr('src',file);}
                            var str = file.split('/');
                            var count = str.length;
                            var strKq = file;
                            if(count>0)
